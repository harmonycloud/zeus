...
public class OperationAuditServiceImpl implements OperationAuditService {
    ...
    private List<String> getTranslateSearch(String groupName, List<String> list, List<String> property, Boolean all, Boolean fuzzyMatching) {
        ...
        // 集合所有字段
        List<String> allPropertyList = matchedTranslateConfigList.stream()
            .map(BeanSysResourceTranslateConfig::getUniqueValue).collect(Collectors.toList());
        // 不携带原信息可能导致部分本无需翻译的关键词丢失
        if (all) {
            allPropertyList.addAll(list);
        }
