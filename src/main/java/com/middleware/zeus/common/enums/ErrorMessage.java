...
public enum ErrorMessage {
    ...
    HELM_INSTALL_GRAFANA_FAILED(211009, "helm install grafana failed", "集群安装grafana失败"),
    ...
    POSTGRESQL_CREATE_USER_FAILED(730019, "postgresql create user failed", "postgresql创建用户失败"),
    POSTGRESQL_DELETE_USER_FAILED(73020, "postgresql delete user failed", "postgresql删除用户失败"),
    POSTGRESQL_UPDATE_USER_FAILED(730021, "postgresql update user failed", "postgresql更新用户失败"),
    POSTGRESQL_GRANT_USER_DATABASE_FAILED(730022, "postgresql grant user database failed", "postgresql赋权用户库权限失败"),
    POSTGRESQL_GRANT_USER_SCHEMA_FAILED(730023, "postgresql grant user schema failed", "postgresql赋权用户模式权限失败"),
    POSTGRESQL_GRANT_USER_TABLE_FAILED(730024, "postgresql grant user table failed", "postgresql赋权用户表权限失败"),
    POSTGRESQL_REVOKE_USER_DATABASE_FAILED(730025, "postgresql revoke user database failed", "postgresql取消赋权用户库权限失败"),
    POSTGRESQL_REVOKE_USER_SCHEMA_FAILED(730026, "postgresql revoke user schema failed", "postgresql取消赋权用户模式权限失败"),
    POSTGRESQL_REVOKE_USER_TABLE_FAILED(730027, "postgresql revoke user table failed", "postgresql取消赋权用户表权限失败"),
    POSTGRESQL_SELECT_FAILED(730028, "postgresql select failed", "postgresql查询数据失败"),
    MIDDLEWARE_API_REQUEST_ERROR(730029, "middleware manager center request error", "中间件管理中心接口请求失败"),
    POSTGRESQL_ADD_TABLE_COLUMN_IS_NULL(730030, "postgresql create table, column could not be null", "postgresql创建表，列信息不能为空"),
    POSTGRESQL_USER_RESET_PASSWORD_FAILED(730031, "postgresql user reset password failed", "重置用户密码失败"),
    POSTGRESQL_SYNC_POD_NOT_EXIST(730032, "postgresql sync pod not exist", "postgresql未存在同步节点，不可切换"),
    // mysql dashboard
